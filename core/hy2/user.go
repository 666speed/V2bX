...
func (v *V2bX) Authenticate(addr net.Addr, auth string, tx uint64) (ok bool, id string) {
	...
	if user, exists := v.users[auth]; exists {
		if !user.IPWhitelistBypass && (!user.IPWhitelistEnabled || !limiter.CheckIPWhitelist(extractIPFromAddr(addr), user.IPWhitelist)) {
			return false, ""
		}
		return true, auth
	}
